...
def add_space_between_e_and_number(input_string):
    # Fixed: use proper backreferences \1 \2 instead of literal "1 2"
...
async def get_search_results(chat_id, query, file_type=None, max_results=10, offset=0, filter=False):
    ...
    if MULTIPLE_DATABASE:
        async for file in col.find(filter).sort('$natural', -1).skip(offset).limit(max_results):
            files.append(file)
        async for file in sec_col.find(filter).sort('$natural', -1).skip(offset).limit(max_results):
            files.append(file)
    else:
        async for file in col.find(filter).sort('$natural', -1).skip(offset).limit(max_results):
            files.append(file)
    ...
    if not MULTIPLE_DATABASE:
        ...
        total_results = await col.count_documents(filter) + await sec_col.count_documents(filter)
...
async def get_bad_files(query, file_type=None, use_filter=False):
    ...
    if not MULTIPLE_DATABASE:
        ...
        files = await col.find(filter_criteria).to_list(length=None)
        files += await sec_col.find(filter_criteria).to_list(length=None)
