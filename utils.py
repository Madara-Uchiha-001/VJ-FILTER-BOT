...
async def is_subscribed(bot, query):
    # AUTH_CHANNEL can now be a single int or a list of ints.
    # We normalise it into a list so the loop below works for both cases.
    ...
    if REQUEST_TO_JOIN_MODE == True and join_db().isActive():
        try:
            ...
            if user and user["user_id"] == query.from_user.id:
                ...
                # Check every channel — user must be in ALL of them
                for channel in channels:
                    try:
                        user_data = await bot.get_chat_member(channel, query.from_user.id)
                    except UserNotParticipant:
                        return False  # Not in this channel, fail immediately
                    except Exception as e:
                        logger.exception(e)
                    else:
                        if user_data.status == enums.ChatMemberStatus.BANNED:
                            return False  # Banned in this channel, fail immediately
                return True  # Passed all channel checks
        except Exception as e:
            logger.exception(e)
            return False
    else:
...
async def verify_user(bot, userid, token):
    ...
    # Store 24hr expiry in MongoDB (persists across restarts)
    await db.set_verify_expiry(user.id)
...
async def check_verification(bot, userid):
    ...
    if not await db.is_user_exist(user.id):
        await db.add_user(user.id, user.first_name)
        await bot.send_message(LOG_CHANNEL, script.LOG_TEXT_P.format(user.id, user.mention))
    # Check MongoDB instead of in-memory dict
    return await db.check_verify_expiry(user.id)
